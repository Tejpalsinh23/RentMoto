...
<div class="container-fluid py-5 px-md-5 animated-fade-in">
    <div class="row">
        ...
        <div class="col-lg-3 mb-4">
            <div class="card card-custom p-3 border-0 shadow-sm" style="background-color: var(--card-bg);">
                ...
                <div class="nav flex-column gap-1 small">
                    ...
                    <a href="{{ route('admin.customers.index') }}" class="admin-nav-link active"><i class="fa-solid fa-users me-2"></i>Customers</a>
        ...
        <!-- Main Content -->
        <div class="col-lg-9">
            <h3 class="fw-bold mb-4 text-dark"><i class="fa-solid fa-users me-2 text-primary"></i>Customer Directory</h3>
                        ...
                        <tbody>
                            @forelse($customers as $c)
                                <tr>
                                    ...
                                    <td><span class="small text-secondary">{{ $c->phone ?? 'N/A' }}</span></td>
                                    <td><span class="small text-muted text-truncate d-inline-block" style="max-width: 250px;">{{ $c->address ?? 'N/A' }}</span></td>
                                    <td><small>{{ $c->created_at->format('Y-m-d') }}</small></td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-muted small py-4">No registered customers found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
