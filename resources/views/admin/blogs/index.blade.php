...
<div class="container-fluid py-5 px-md-5 animated-fade-in">
    <div class="row">
        ...
        <div class="col-lg-3 mb-4">
            <div class="card card-custom p-3 border-0 shadow-sm" style="background-color: var(--card-bg);">
                ...
                <div class="nav flex-column gap-1 small">
                    ...
                    <a href="{{ route('admin.blogs.index') }}" class="admin-nav-link active"><i class="fa-solid fa-blog me-2"></i>Blog Posts</a>
        ...
        <!-- Main Content -->
        <div class="col-lg-9">
            <h3 class="fw-bold mb-4 text-dark"><i class="fa-solid fa-feather-alt me-2 text-primary"></i>Manage Blog Posts</h3>

            <div class="row g-4">
                <!-- Left: Create Blog Form -->
                <div class="col-md-4">
                    <div class="card card-custom p-4 border-0 shadow-sm" style="background-color: var(--card-bg);">
                        <h5 class="fw-bold mb-3">Create Blog</h5>
                        <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label small fw-semibold text-muted">Post Title</label>
                                <input type="text" name="title" class="form-control border-secondary-subtle" placeholder="e.g. Travel Guide" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label class="form-label small fw-semibold text-muted">Post Body Content</label>
                                <textarea name="content" class="form-control border-secondary-subtle" rows="5" placeholder="Write post body here..." required></textarea>
                            </div>
                            <div class="mb-4">
                            ...
                            <table class="table align-middle text-start table-hover border-top border-secondary-subtle">
                                ...
                                <tbody>
                                    @forelse($blogs as $blog)
                                        <tr>
                                            <td><span class="fw-bold text-dark">{{ $blog->title }}</span></td>
                                            <td><span class="small">{{ $blog->category->name }}</span></td>
                                            <td><span class="small text-muted">{{ $blog->views }} views</span></td>
                                            <td><small>{{ $blog->created_at->format('Y-m-d') }}</small></td>
                                            <td class="text-end">
                                                <form action="{{ route('admin.blogs.destroy', $blog->id) }}" method="POST" onsubmit="return confirm('Delete this blog post?');" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center text-muted small py-4">No blog posts published yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
