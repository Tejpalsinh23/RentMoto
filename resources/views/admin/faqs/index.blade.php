...
<div class="container-fluid py-5 px-md-5 animated-fade-in">
    <div class="row">
        ...
        <div class="col-lg-3 mb-4">
            <div class="card card-custom p-3 border-0 shadow-sm" style="background-color: var(--card-bg);">
                ...
                <div class="nav flex-column gap-1 small">
                    ...
                    <a href="{{ route('admin.faqs.index') }}" class="admin-nav-link active"><i class="fa-solid fa-question-circle me-2"></i>FAQs</a>
        ...
        <!-- Main Content -->
        <div class="col-lg-9">
            <h3 class="fw-bold mb-4 text-dark"><i class="fa-solid fa-question-circle me-2 text-primary"></i>Frequently Asked Questions</h3>

            <div class="row g-4">
                <!-- Left: Create FAQ Form -->
                <div class="col-md-4">
                    <div class="card card-custom p-4 border-0 shadow-sm" style="background-color: var(--card-bg);">
                        <h5 class="fw-bold mb-3">Create FAQ</h5>
                        <form action="{{ route('admin.faqs.store') }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label small fw-semibold text-muted">Question</label>
                                <input type="text" name="question" class="form-control border-secondary-subtle" placeholder="Question text" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label small fw-semibold text-muted">Answer</label>
                                <textarea name="answer" class="form-control border-secondary-subtle" rows="4" placeholder="Answer content" required></textarea>
                            </div>
                            <div class="mb-4">
                                <label class="form-label small fw-semibold text-muted">Sorting Order</label>
                                <input type="number" name="order_num" class="form-control border-secondary-subtle" value="0">
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle text-start table-hover border-top border-secondary-subtle">
                                ...
                                <tbody>
                                    @forelse($faqs as $faq)
                                        <tr>
                                            <td><span class="badge bg-secondary-subtle text-muted">{{ $faq->order_num }}</span></td>
                                            <td><span class="fw-bold text-dark">{{ $faq->question }}</span></td>
                                            <td><span class="small text-muted text-truncate d-inline-block" style="max-width: 300px;">{{ $faq->answer }}</span></td>
                                            <td class="text-end">
                                                <form action="{{ route('admin.faqs.destroy', $faq->id) }}" method="POST" onsubmit="return confirm('Delete this FAQ accordion?');" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center text-muted small py-4">No FAQs added yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
